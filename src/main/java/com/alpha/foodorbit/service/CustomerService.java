...
//        address.setAddressType(dto.ge);
       ...
       emailService.sendRegistrationEmail(savedCustomer.getMailid(),savedCustomer.getName());
...
//          PaymentIntent paymentIntent =  stripeService.createPaymentIntent(stripeAmount);
          Payment payment=new Payment();
...
//          payment.setStripePaymentId(paymentIntent.getId());
          ...
          ResponseStructure<String> rs= new ResponseStructure<>();
          rs.setStatuscode(200);
          rs.setMessage("Stripe Payment Initiated");
//          rs.setData(paymentIntent.getClientSecret());
